...
def getResistanceMatrix(mesh,materialFactor):
    ...
    neighbourPairs = np.where(getNeighbourhoodMatrix(mesh))
    ...
    for edgeInd in range(len(meshEdgesNonUnique[0])):
        ...
        if overlappingTriangles:
            ...
            for overlappTriangle in overlappingTriangles:
                ...
                for i in range(len(mesh.neighbourcurrents[nodeInd1])):#hier werden die falschen dinge kombiniert
                    primaryCurrent += mesh.neighbourcurrents[nodeInd1][i]*firstNodeTrianglePosition[i]
                for i in range(len(mesh.neighbourcurrents[nodeInd2])):#hier werden die falschen dinge kombiniert
